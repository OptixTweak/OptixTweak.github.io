...
(function(){
  ...
  function renderList(products){
    ...
    if(!list) return;
    list.innerHTML = products.map(p=>`<div class="item"><div><strong>${escapeHtml(p.name)}</strong><div class="small">${escapeHtml(p.short||'')}</div></div><div><button data-id="${_u(p.id)}" class="edit ghost">Edit</button> <button data-id="${_u(p.id)}" class="del ghost">Delete</button></div></div>`).join('');
    // attach handlers
  ...
  function openEditor(id){
    ...
    editor.innerHTML = `
      ...
      <label>Price<input id="e-price" type="number" class="input" value="${Number(p.price)||0}"></label>
      <label>Images (comma separated)<input id="e-images" class="input" value="${escapeHtml((p.images||[]).join(','))}"></label>
